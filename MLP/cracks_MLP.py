...
plt.plot(range(final_epochs), train_acc_list, label='Train Accuracy', color='blue')
plt.plot(range(final_epochs), test_acc_list, label='Test Accuracy', color='orange')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
...
# ---------- Predict on New Sample Dataset ----------
new_data = pd.read_csv("C:\\Users\\94772\\Desktop\\FYP\\Samples\\new_dataTEST.csv")
new_data.columns = ['Capacitance', 'Humidity', 'Temperature', 'Thickness', 'CrackType']
new_data['CrackType'] = label_encoder.transform(new_data['CrackType'])  # Use existing encoder

X_new = new_data[['Capacitance', 'Humidity', 'Temperature', 'Thickness']].values
y_new = new_data['CrackType'].values
...
with torch.no_grad():
    ...
    new_classes = torch.argmax(new_preds, dim=1)

new_accuracy = accuracy_score(y_new, new_classes.numpy())
print(f"\n🆕 Accuracy on New Sample Dataset: {new_accuracy * 100:.2f}%")

joblib.dump(scaler, "scaler.pkl")
joblib.dump(label_encoder, "label_encoder.pkl")

torch.save(model.state_dict(), "C:\\Users\\94772\\Desktop\\FYP\\PythonApp\\models\\MLP_Model.pth")
